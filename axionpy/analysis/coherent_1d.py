...
It works the same as the "coherent" time domain analysis except the model assumes
...
def compute_coefficients(m, t, y, **kwargs):
    ...
    Returns
    ---------------
    Az, Bz: astropy.Quantity
           The A and B coefficients such that 
           y = (Az*cos(m*t) - Bz*sin(m*t))*m_dot_z is the least-squares solution
           where m_dot_z is the measurement axis projected along the zhat xyz basis vector.
...
def frequentist_upper_limit(az, bz, s, confidence=0.95, gmax=None, llmax=None, g_scale=None):
    ...
    """
    if gmax is None:
        gmax, llmax = maximize_likelihood(az, bz, s, g_scale=g_scale)
    elif llmax is None:
